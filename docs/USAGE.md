...
- 关于页面的更新检查地址可修改；管理员可编辑更新命令模板，默认模板为 `IMAGE=ghcr.io/liyan-lucky/rustdesk-api-server-pro:{version} EXPECTED_VERSION={version} /vol1/docker/rustdesk-api-server-pro/update-rustdesk-api.sh`，复制时会用最新版本替换 `{version}`
