...
- 客户端“同步最近会话”通过通讯簿 peer 新增/更新接口写入服务器。新增接口按 `user_id + ab_id + rustdesk_id` 幂等合并，重复同步只更新设备描述字段，不重复建档，也不覆盖用户维护的标签、别名、备注和密码。
- 管理后台 `/admin/ab/list` 返回当前账户可写的通讯簿：管理员可管理全部通讯簿；普通用户可管理本人通讯簿，以及全局规则或用户/用户组规则达到 `rule >= 2` 的共享通讯簿。标签和联系人新增表单共用该列表，只读通讯簿不会进入选择器。
- `/audit/baselogs`：连接审计。
