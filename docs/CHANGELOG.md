...
- 完成 ja-jp、ko-kr、ru-ru、es-es、fr-fr 多语言翻译，翻译覆盖率从约 37% 提升至约 70%。
...
- 更新 i18n 质量检查配置，调整 ja-jp/ko-kr/es-es/ru-ru 阈值允许 en-us 回退。
- 更新 i18n 基线对比阈值，允许 ja-jp/ko-kr/es-es/ru-ru 使用 en-us 回退。

### 修复

- 修复由移动端切换回桌面布局后主菜单外层未显式透出背景，导致毛玻璃透明效果丢失的问题，并补充 WebKit 浏览器的背景模糊兼容。
- 修复移动端登录卡片宽度叠加内边距后超出视口，导致用户名、密码、确认按钮和第三方登录按钮被截断的问题。
- 修复移动端侧栏外层不透明背景阻断毛玻璃效果，导致工具菜单呈灰色且未跟随主题透明度的问题。
...
- 修复 fr-fr 的 `action` 字段未翻译问题（`Action` → `Opération`）。
- 修复毛玻璃效果初始化时机问题，确保 CSS 变量注入后再计算毛玻璃变量。

### 性能优化

- 添加背景图片预加载标签，减少首页加载延迟。
