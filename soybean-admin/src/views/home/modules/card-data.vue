...
const activeCard = ref<CardData | null>(null);
...
const activeCardDesc = computed(() => {
  ...
  if (!key) return '';
  if (key === 'userCount') return '展示当前系统内用户总数。';
  if (key === 'deviceCount') return '展示当前系统内设备总数。';
  if (key === 'onlineCount') return '展示基于心跳统计得到的在线设备数量。';
  return '展示来自审计日志的访问次数统计。';
});

const activeCardListTitle = computed(() => {
  const key = activeCard.value?.key;
  if (!key) return '';
  if (key === 'userCount') return '最近用户';
  if (key === 'visitCount') return '最近访问记录';
  return '最近设备';
});
...
<template>
  <NCard :bordered="false" size="small" class="card-wrapper">
    ...
    <NDrawer v-model:show="drawerVisible" :width="420" placement="right" :trap-focus="false">
      <NDrawerContent v-if="activeCard" :title="`${activeCard.title}详情`" closable>
        <div class="drawer-body">
          ...
          <p class="drawer-desc">{{ activeCardDesc }}</p>
