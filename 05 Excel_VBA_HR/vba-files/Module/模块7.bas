...
' 功能：导出考勤清单
' 1) 选择需要打开的Excel文件
' 2) 删除目标文件中名为"考勤清单"的Sheet（若存在）
' 3) 将当前文件的"考勤清单"复制到目标文件
' 4) 保持工作表位置与源文件一致
' =======================================================
Sub 导出考勤清单()
    ...
    ' 获取源工作表
    Set wsSource = wbSource.Worksheets("考勤清单")
    ...
    On Error Resume Next
    Set wsOld = wbTarget.Worksheets("考勤清单")
    On Error GoTo ErrorHandler
    If Not wsOld Is Nothing Then
        tmpOldSheetName = "__bak_kqqd_" & Format(Now, "hhmmss")
